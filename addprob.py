...
def addprob_submit():
	if (session.get('username') in configs.admin):
		prob = {"num": request.form['num'], "unique_id": request.form['uniqid'], "display_name": request.form['title'], "by": request.form['by'], "limit_time": request.form['tlimit'], "limit_memory": request.form['mlimit'], "desc": request.form['desc'], "input": request.form['input'], "output": request.form['output'], "exin": request.form['input_ex'], "exout": request.form['output_ex'], 'solved': 0, 'submits': 0, 'diff': 0}
